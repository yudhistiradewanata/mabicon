...
class TransactionManagementController extends MY_Controller
{
    ...
    public function approveTopUp()
    {
        ...
        if ($topUpRequest && $topUpRequest->status == 'pending') {
            ...
            $this->balanceModel->insertDebit($topUpRequest->user_id, $topUpRequest->topup_amount, 'top_up_requests', $topUpRequest->id, 'Top-up approved of '.$topUpRequest->topup_amount);
    ...
    public function rejectWithdrawal()
    {
        ...
        if ($withdrawalRequest && $withdrawalRequest->status == 'pending') {
            ...
            $this->balanceModel->insertDebit($withdrawalRequest->user_id, $withdrawalRequest->withdrawal_amount, 'withdrawal_requests', $withdrawalRequest->id, 'Withdrawal rejected of '.$withdrawalRequest->withdrawal_amount);
